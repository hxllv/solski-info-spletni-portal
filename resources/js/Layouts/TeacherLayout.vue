...
<template>
    <AppLayout :title="title" :permission="permission">
        ...
        <ul class="m-2 max-w-7xl mt-0 px-2 sm:px-6 lg:px-8 mx-auto flex flex-wrap text-sm font-medium text-center text-gray-500" v-if="!backButtonURL && classId">
            <li class="mr-2" v-if="permission.includes('users.view')">
                <Tab :href="route('gradebook', {classId: classId})" :active="route().current('gradebook')">
                    Redovalnica
                </Tab>
            </li>
            <li class="mr-2" v-if="permission.includes('users.view')">
