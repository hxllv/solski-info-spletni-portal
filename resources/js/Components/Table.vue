                ...
                <tbody class="text-xs md:text-sm lg:text-base">
                    <tr v-for="row in data.data" class="bg-white border-b hover:bg-gray-100">
                        <td class="border-r max-w-fit" v-if="allowMultiActions">
                            <div class="flex justify-center px-1">
                                <Checkbox v-model:checked="idObj[row.id]" @change="selectedChange"/>    
                            </div>
                        </td>
                        <td class="px-6 py-4 border-r" v-for="val in sortedAs">{{ row[val] }}</td>
                        <td class="text-right max-w-fit" v-if="allowEdit || allowDelete || detailsURL">
                            <div class="flex justify-center px-1">
                                <Link :href="route(detailsURL, row.id)" v-if="detailsURL">
                                    <font-awesome-icon :icon="['fas', 'circle-info']" class="text-gray-700 px-2" />
                                </Link>
