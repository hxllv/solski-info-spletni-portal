...
const helperArrays = computed(() => {
    return {
        ...
        last5: [
            props.data.last_page - 4,
            props.data.last_page - 3,
            props.data.last_page - 2,
...
const getClass = (pageNoTemp) => {
    return pageNoTemp == props.data.current_page
        ? `${classes} text-white bg-blue-500 cursor-default pointer-events-none`
...
<template>
    <div
        class="flex items-center justify-between border-gray-200 bg-white px-4 py-3 sm:px-6 col-span-3"
    >
            ...
            <div>
                ...
                >
                    ...
                    <Link
                        ...
                        preserve-state
                        :href="route(routeName, ...routeParams)"
                        :data="{ [pageName]: isAtData('start'), ...query }"
                        :class="isAt('start')"
                    ...
                    <template
                        v-if="data.last_page < 6"
                        v-for="(value, index) in Array(data.last_page)"
                    >
                        <Link
                            preserve-scroll
                            preserve-state
                            :href="route(routeName, ...routeParams)"
                            :data="{ [pageName]: index + 1, ...query }"
                    ...
                    <template v-if="data.last_page >= 6">
                        <template
                            ...
                            v-for="val in helperArrays.first5"
                        >
                            <Link
                                preserve-scroll
                                preserve-state
                                :href="route(routeName, ...routeParams)"
                                :data="{ [pageName]: val, ...query }"
                                :class="getClass(val)"
                            >
                                {{ val }}
                            </Link>
                        ...
                        <template
                            ...
                            "
                            v-for="val in helperArrays.minus2toPlus2"
                        >
                            <Link
                                preserve-scroll
                                preserve-state
                        ...
                        <template
                            ...
                            v-for="val in helperArrays.last5"
                        >
                            <Link
                                preserve-scroll
                                preserve-state
                    ...
                    <Link
                        ...
                        preserve-state
                        :href="route(routeName, ...routeParams)"
                        :data="{ [pageName]: isAtData('end'), ...query }"
                        :class="isAt('end')"
                        class="relative inline-flex items-center px-2 py-2 ring-1 ring-inset ring-gray-300"
                    >
                        <span class="sr-only">Previous</span>
                    ...
                    <Link
                        ...
                        preserve-state
                        :href="route(routeName, ...routeParams)"
                        :data="{ [pageName]: data.last_page, ...query }"
                        :class="isAt('end')"
