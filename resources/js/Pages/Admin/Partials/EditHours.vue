...
const editHours = () => {
    //remove empty rows
    parsedHours.value = parsedHours.value.filter(item => item.name !== '' && item.to !== '' && item.from !== '')
...
<template>
    ...
    <DialogModal :show="editingHours" @close="closeHoursModal"> 
        ...
        <template #content>
            <form @submit.prevent="">
                <div
                    class="overflow-x-auto grid grid-cols-3 sm:rounded-t-md"
                >
                    <table class="table-fixed mt-0 col-span-3">
                            ...
                            <tr v-for="(hour, index) in parsedHours">
                                <td>
                                    <TextInput
                                        ...
                                        @blur="parsedHours.sort((a, b) => a.index - b.index)"
                                    />
                                </td>
                                <td>
                                    <TextInput
                    ...
                    <div class="col-span-3">
                        <PrimaryButton
                            class="px-2 py-1 m-2"
                            @click="parsedHours.push({name: '', to: '', from: ''})"
                        >
                            Dodaj Vrstico
                        </PrimaryButton>
                    </div>
                    ...
                    <div class="col-span-3">
                        <InputError :message="formHours.errors.value" class="mt-2" />
                    </div>
                </div>
                <p class="py-2">Prazne vrstice bodo ignorirane, tudi v primeru, ko niso na koncu!</p>
                <p>Indeks:</p>
                <ul class="list-disc py-2 pl-4">
                    <li>Vnosi urnika uporabljajo vrednost "indeks" za določitev, v katero uro spadajo;</li>
                    <li>Priporočeno je, da se teh vrednosti ne spreminja, če že imate določene urnike;</li>
                    <li>Če imate vnos urnika, z določenim indeksom, in ta indeks s tega seznama odstranite, se vnos ne bo prikazal na urniku;</li>
                    <li>Prav tako ni priporočeno, da imata dve (ali več) uri isti indeks, ker bo to vnose podvojilo;</li>
